...
const fD = d => { if(!d) return '—'; const s=String(d).trim(); const clean=s.includes('T')||s.includes(' ')?s.replace(' ','T').split('T')[0]:s; return new Date(clean+'T06:00:00Z').toLocaleDateString('es-HN',{day:'2-digit',month:'2-digit',year:'numeric',timeZone:'America/Tegucigalpa'}); };
...
const NAV = [
  ...
  { view:'series',       label:'Series Factura',      roles:['admin'] },
...
function renderView(view) {
  switch(view) {
    ...
    case 'series':    renderSeriesLista(); break;
...
async function reporteMasterVentas() {
  ...
  const _mExento  = x => { const si=(x.isv15||0)===0&&(x.isv18||0)===0; return si&&(x.importe_exento||0)===0?x.total||0:x.importe_exento||0; };
  const _mGravado = x => { const si=(x.isv15||0)===0&&(x.isv18||0)===0; return si&&(x.importe_exento||0)===0?0:x.importe_gravado||0; };
  const totGravM = sales.reduce((s,x)=>s+_mGravado(x),0);
  const totExenM = sales.reduce((s,x)=>s+_mExento(x),0);
  const totIsvM  = sales.reduce((s,x)=>s+(x.isv15||0)+(x.isv18||0),0);
  const totDescM = sales.reduce((s,x)=>s+(x.descuento||0),0);
  const hGM  = totGravM > 0;
  const hEM  = totExenM > 0;
  const hIM  = totIsvM  > 0;
  const hDM  = totDescM > 0;
...
<table><thead><tr>
  <th>N° Factura</th><th>Fecha</th><th>Cliente</th><th>RTN</th>
  ${hGM?'<th style="text-align:right">Gravado</th>'    :''}
  ${hEM?'<th style="text-align:right">Exento</th>'     :''}
  ${hIM?'<th style="text-align:right">ISV</th>'        :''}
  ${hDM?'<th style="text-align:right">Desc.</th>'      :''}
  <th style="text-align:right">Total</th>
</tr></thead>
<tbody>${sales.map(s=>`<tr>
  <td style="font-family:monospace">${s.numero_factura}</td>
  <td>${fD(s.fecha)}</td>
  <td>${s.cliente_nombre||'—'}</td>
  <td>${s.cliente_rtn||'—'}</td>
  ${hGM?`<td style="text-align:right;color:${_mGravado(s)>0?'inherit':'#cbd5e1'}">${_mGravado(s)>0?fL(_mGravado(s)):'—'}</td>` :''}
  ${hEM?`<td style="text-align:right;color:${_mExento(s)>0?'#15803d':'#cbd5e1'}">${_mExento(s)>0?fL(_mExento(s)):'—'}</td>`  :''}
  ${hIM?`<td style="text-align:right;color:${((s.isv15||0)+(s.isv18||0))>0?'inherit':'#cbd5e1'}">${((s.isv15||0)+(s.isv18||0))>0?fL((s.isv15||0)+(s.isv18||0)):'—'}</td>`:''}
  ${hDM?`<td style="text-align:right">${fL(s.descuento||0)}</td>`       :''}
  <td style="text-align:right;font-weight:700">${fL(s.total)}</td>
</tr>`).join('')}</tbody>
<tfoot><tr class="total-row">
  <td colspan="4">TOTALES (${sales.length} facturas)</td>
  ${hGM?`<td style="text-align:right">${fL(totGravM)}</td>`:''}
  ${hEM?`<td style="text-align:right">${fL(totExenM)}</td>`:''}
  ${hIM?`<td style="text-align:right">${fL(totIsvM)}</td>` :''}
  ${hDM?`<td style="text-align:right">${fL(totDescM)}</td>`:''}
  <td style="text-align:right">${fL(total)}</td>
</tr></tfoot></table>
...
function printTicket(sale) {
  ...
  const itemsRows = sale.items.map(i => {
    const name = (i.nombre||'').length > 24 ? (i.nombre||'').substring(0,24)+'.' : (i.nombre||'');
    return `<tr><td colspan="2" style="padding:2px 0;font-size:12px;font-weight:700">${name}</td></tr>
    <tr><td style="padding:0 0 4px;font-size:12px;color:#333">${i.cantidad} x L.${(i.precio||0).toFixed(2)}</td>
    <td style="padding:0 0 4px;font-size:12px;text-align:right;font-weight:700">L.${((i.precio||0)*(i.cantidad||0)).toFixed(2)}</td></tr>`;
  }).join('');
  const sep = `<tr><td colspan="2"><div style="border-top:1px dashed #999;margin:6px 0"></div></td></tr>`;
  const logoSrc = b.logo || '';
  // ── Ticket 76mm · Arial 12px · Bold en títulos · Use Font Script (legibilidad) ──
...
body{
  ...
  max-width:70mm;
  padding:2mm 1.5mm;
  /* Use Font Script: mejora renderizado en impresoras térmicas */
...
@media print{
  ...
  body{padding:1.5mm 1mm;width:70mm;max-width:70mm;font-size:12px}
...
.total-line td{font-size:14px;font-weight:700;padding-top:5px}
</style></head><body>
${logoSrc?`<div style="text-align:center;margin-bottom:6px"><img src="${logoSrc}" style="max-width:62mm;max-height:20mm;object-fit:contain"/></div>`:''}
<div class="c b" style="font-size:13px">${b.nombre||'METRIC POS'}</div>
<div class="c" style="font-size:11px">RTN: ${b.rtn||''}</div>
<div class="c" style="font-size:11px">${b.direccion||''}</div>
<div class="c" style="font-size:11px">Tel: ${b.telefono||''}</div>
<div class="sep"></div>
<div class="c b" style="font-size:14px">FACTURA</div>
<div class="c b" style="font-size:12px">${sale.numero_factura}</div>
<div class="c" style="font-size:11px">${fechaStr} ${horaStr}</div>
<div class="sep"></div>
<div><span class="b">Cliente:</span> ${sale.cliente?.nombre||'—'}</div>
${sale.cliente?.rtn?`<div><span class="b">RTN:</span> ${sale.cliente.rtn}</div>`:''}
<div class="sep"></div>
<table>${itemsRows}${sep}
<tr><td>Total Neto</td><td style="text-align:right">L. ${((sale.subtotal||0)-(sale.isv||0)-(sale.isv18||0)).toFixed(2)}</td></tr>
${(sale.descuento||0)>0?`<tr><td>Desc. y Rebajas</td><td style="text-align:right">-L. ${(sale.descuento||0).toFixed(2)}</td></tr>`:''}
<tr><td>Imp. Gravado</td><td style="text-align:right">L. ${(sale.importeGravado||0).toFixed(2)}</td></tr>
<tr><td>Imp. Exento</td><td style="text-align:right">L. ${(sale.importeExento||0).toFixed(2)}</td></tr>
<tr><td>Imp. Exonerado</td><td style="text-align:right">L. ${(sale.exonerado?(sale.importeExento||0):0).toFixed(2)}</td></tr>
${(sale.isv||0)>0?`<tr><td>ISV 15%</td><td style="text-align:right">L. ${(sale.isv||0).toFixed(2)}</td></tr>`:''}
${(sale.isv18||0)>0?`<tr><td>ISV 18%</td><td style="text-align:right">L. ${(sale.isv18||0).toFixed(2)}</td></tr>`:''}
${sep}
<tr class="total-line"><td class="b">TOTAL</td><td style="text-align:right">L. ${(sale.total||0).toFixed(2)}</td></tr>
${sep}
<tr><td class="b">F. Pago</td><td style="text-align:right">${sale.formaPago==='efectivo'?'Efectivo':sale.formaPago==='tarjeta'?'Tarjeta':sale.formaPago==='credito'?'Crédito':'Transferencia'}</td></tr>
${sale.formaPago==='efectivo'&&(sale.montoRecibido||0)>0?`<tr><td>Recibido</td><td style="text-align:right">L. ${(sale.montoRecibido||0).toFixed(2)}</td></tr><tr><td class="b">Cambio</td><td style="text-align:right;font-weight:700">L. ${(sale.cambio||0).toFixed(2)}</td></tr>`:''}
</table>
<div style="font-size:11px;text-align:center;margin-top:3px">${moneyToWords(sale.total||0)}</div>
<div class="sep"></div>
<div style="font-size:11px"><span class="b">CAI:</span> ${b.cai||''}</div>
<div style="font-size:11px"><span class="b">Rango:</span> ${b.rango_ini||''}</div>
<div style="font-size:11px;padding-left:8mm">AL ${b.rango_fin||''}</div>
<div style="font-size:11px"><span class="b">F.Limite:</span> ${fD(b.fecha_limite||'')}</div>
<div class="sep"></div>
<div class="c" style="font-size:11px">Original / Cliente &nbsp; Copia / Obligado</div>
<div class="c b" style="font-size:11px;margin-top:4px">Powered by Metric POS</div>
...
async function processFacInvoice() {
  ...
  let isv15 = 0, isv18 = 0, importeGravado = 0, importeExentoItems = 0;
  if (!facExonerado) {
    facCart.forEach(item => {
      const base = item.precio * item.cantidad;
      if (!item.gravado) {
        // Producto exento: suma al importe exento
        importeExentoItems += base;
        return;
      }
      const tasa = item.tasaIsv || 15;
      const isvItem = base - base / (1 + tasa / 100);
  ...
  // Exonerado: todo el monto va a exento. Exento normal: solo items sin gravado
  const importeExento = facExonerado ? afterDisc : importeExentoItems;
...
async function reporteLibroFiscal() {
  ...
  const b = BRANCH||{};
  // Para ventas sin ISV guardadas como importe_gravado: recalcular exento real
  const _vExento = v => {
    const sinIsv = (v.isv15||0)===0 && (v.isv18||0)===0;
    if (sinIsv && (v.importe_exento||0)===0) return v.total||0;
    return v.importe_exento||0;
  };
  const _vGravado = v => {
    const sinIsv = (v.isv15||0)===0 && (v.isv18||0)===0;
    if (sinIsv && (v.importe_exento||0)===0) return 0;
    return v.importe_gravado||0;
  };
  const totGrav   = ventas.reduce((s,v)=>s+_vGravado(v),0);
  const totExento = ventas.reduce((s,v)=>s+_vExento(v),0);
...
${(()=>{
  const hG  = totGrav   > 0;
  const hE  = totExento > 0;
  const hI5 = totIsv15  > 0;
  const hI8 = totIsv18  > 0;
  const hD  = totDesc   > 0;
  return `<table><thead><tr>
    <th>#</th><th>Fecha</th><th>N° Factura</th><th>Cliente</th><th>RTN Cliente</th>
    ${hG ?'<th style="text-align:right">Imp.Gravado</th>'  :''}
    ${hE ?'<th style="text-align:right">Exento</th>'       :''}
    ${hI5?'<th style="text-align:right">ISV 15%</th>'      :''}
    ${hI8?'<th style="text-align:right">ISV 18%</th>'      :''}
    ${hD ?'<th style="text-align:right">Descuento</th>'    :''}
    <th style="text-align:right">Total</th>
  ...
  <tbody>${ventas.map((v,i)=>`<tr ${v.estado==='anulada'?'style="text-decoration:line-through;color:#94a3b8"':''}>
    ...
    ${hG ?`<td style="text-align:right;color:${_vGravado(v)>0?'inherit':'#cbd5e1'}">${_vGravado(v)>0?fL(_vGravado(v)):'—'}</td>`:''}
    ${hE ?`<td style="text-align:right;color:${_vExento(v)>0?'#15803d':'#cbd5e1'}">${_vExento(v)>0?fL(_vExento(v)):'—'}</td>` :''}
    ${hI5?`<td style="text-align:right;color:${(v.isv15||0)>0?'inherit':'#cbd5e1'}">${(v.isv15||0)>0?fL(v.isv15):'—'}</td>`  :''}
    ${hI8?`<td style="text-align:right;color:${(v.isv18||0)>0?'inherit':'#cbd5e1'}">${(v.isv18||0)>0?fL(v.isv18):'—'}</td>`  :''}
    ${hD ?`<td style="text-align:right">${fL(v.descuento||0)}</td>`      :''}
  ...
  <tfoot><tr class="total-row">
    ...
    ${hG ?`<td style="text-align:right">${fL(totGrav)}</td>`   :''}
    ${hE ?`<td style="text-align:right">${fL(totExento)}</td>` :''}
    ${hI5?`<td style="text-align:right">${fL(totIsv15)}</td>`  :''}
    ${hI8?`<td style="text-align:right">${fL(totIsv18)}</td>`  :''}
    ${hD ?`<td style="text-align:right">${fL(totDesc)}</td>`   :''}
    <td style="text-align:right">${fL(totTotal)}</td>
    <td colspan="2"></td>
  </tr></tfoot></table>`;
})()}
...
// ── Recalcular ventas históricas como exentas ─────────────────────────────────
async function recalcularVentasExentas() {
  if (!confirm(
    '¿Recalcular todas las ventas históricas como Exentas?\n\n' +
    '• Los totales NO cambian\n' +
    '• Se elimina el ISV registrado\n' +
    '• Se marca el monto como Exento\n\n' +
    'Ejecutar solo si TODOS los productos son exentos.\n¿Confirmar?'
  )) return;
  try {
    const r = await POST('/admin/recalcular_ventas_exentas', {});
    alert(`✅ ${r.mensaje}`);
  } catch(e) { alert('Error: ' + e.message); }
}
