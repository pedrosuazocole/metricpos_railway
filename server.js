...
app.post('/api/turnos/abrir',auth(),(req,res)=>{
  try {
    const suc=req.user.sucursal_id;
    const existente=get(`SELECT id FROM turnos WHERE sucursal_id=? AND estado='abierto'`,[suc]);
    ...
    const id=uuid();
    run(`INSERT INTO turnos(id,sucursal_id,usuario_id,fondo_inicial,sin_fondo,notas)VALUES(?,?,?,?,?,?)`,[id,suc,req.user.id,fondo_inicial||0,sin_fondo?1:0,notas||'']);
    saveDB(); res.json({id});
...
// ── MIGRACIÓN: Recalcular ventas históricas como exentas ──────────────────────
// Ejecutar UNA SOLA VEZ desde el panel de Configuración (solo admin)
app.post('/api/admin/recalcular_ventas_exentas', auth(['admin']), (req, res) => {
  try {
    // Obtener todas las ventas emitidas que tienen isv15 > 0
    const ventas = all(`SELECT id, subtotal, descuento, isv15, isv18, total
      FROM ventas WHERE estado='emitida' AND (isv15>0 OR isv18>0)`);
    let actualizadas = 0;
    ventas.forEach(v => {
      // Recalcular: el total ya es correcto, pero importe_gravado → 0
      // importe_exento → total, isv15 → 0, isv18 → 0
      run(`UPDATE ventas SET
        importe_gravado = 0,
    ...
    saveDB();
    res.json({ ok: 1, actualizadas, mensaje: `${actualizadas} ventas recalculadas como exentas` });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
